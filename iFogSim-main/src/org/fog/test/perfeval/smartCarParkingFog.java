...
public class smartCarParkingFog {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("this is brokerId:  "+broker.getId()+"        and this is appid:    "+appId+ "      *******Hossein");

			createFogDevices(broker.getId(), appId);
			Controller controller = null;
			// ******module is VM and we can create multi-VM in each fog node. each VM can
			// process tasks.******(Hossein)
			System.out.println("this is name of fog devices (4):    " + fogDevices + "      *******Hossein");

			ModuleMapping moduleMapping = ModuleMapping.createModuleMapping(); // initializing a module mapping
			System.out.println("this is name of broker Ids:    " + broker + "      *******Hossein");
			System.out.println("first loop  ******hossein");

			for (FogDevice device : fogDevices) {
				if (device.getName().startsWith("c")) { // names of all Smart Cameras start with 'm'
					moduleMapping.addModuleToDevice("picture-capture", device.getName()); // fixing 1 instance of the
			...
			for (FogDevice device : fogDevices) {
				if (device.getName().startsWith("a")) { // names of all fog devices start with 'a'
					moduleMapping.addModuleToDevice("slot-detector", device.getName()); // fixing 1 instance of the
																						// Motion Detector module to
																						// each Smart Camera
				}
			}
			// moduleMapping.addModuleToDevice("user_interface", "cloud"); // fixing
			// instances of User Interface module in the Cloud
			System.out.println("last if  ******hossein");
			if (CLOUD) {
				System.out.println("last if  ******hossein");

				// if the mode of deployment is cloud-based
				moduleMapping.addModuleToDevice("picture-capture", "cloud"); // placing all instances of Object Detector
																				// module in the Cloud
				moduleMapping.addModuleToDevice("slot-detector", "cloud"); // placing all instances of Object Tracker
																			// module in the Cloud
			}
	...
	private static void createFogDevices(int userId, String appId) {
		...
		System.out.println("the proxyId is equal to:   " + proxy.getId()+"     ******hossein" );
	...
	private static FogDevice addCamera(String id, int userId, String appId, int parentId) {
		...
		Sensor sensor = new Sensor("s-" + id, "CAMERA", userId, appId,
				new DeterministicDistribution(CAM_TRANSMISSION_TIME)); // inter-transmission time of camera (sensor)
																		// follows a deterministic distribution
		sensors.add(sensor);
		Actuator ptz = new Actuator("ptz-" + id, userId, appId, "PTZ_CONTROL");
		actuators.add(ptz);
		sensor.setGatewayDeviceId(camera.getId());
		sensor.setLatency(40.0); // latency of connection between camera (sensor) and the parent Smart Camera is
									// 1 ms
	...
	private static Application createApplication(String appId, int userId) {
		 ...
		 */
		application.addAppEdge("CAMERA", "picture-capture", 1000, 500, "CAMERA", Tuple.UP, AppEdge.SENSOR); // adding
																											// edge from
																											// CAMERA
																											// (sensor)
																											// to Motion
